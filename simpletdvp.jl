...
# psi0 = productMPS(sites, "Dn")
# psi_t = simpletdvp(H, psi0, ttot, dt; cutoff = cutoff);
# # ##
# # gate = exp(0.25im * π * op("Sy", sites[10]))
# # psi0_2 = apply(gate, psi0_1)
# # psi_t_2 = simpletdvp(H, psi0_2, ttot, dt; cutoff = cutoff);
...
# ##
# t_list = Vector(0:dt:ttot)
# obs_list = [meas_Sx(psi_t_2[nt], n) - meas_Sx(psi_t_1[nt], n) for n = 1:N, nt = 1:length(t_list)]
# heatmap(1:N, t_list, obs_list)
# ##
# t_list = Vector(0:dt:ttot)
# obs_list = [meas_Sz(psi_t_1[nt], n) for n = 1:N, nt = 1:length(t_list)]
# heatmap(1:N, t_list, obs_list)
# ##
# plot(t_list, obs_list[1, :])
